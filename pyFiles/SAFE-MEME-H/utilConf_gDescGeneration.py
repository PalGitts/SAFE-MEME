...
class CustomDatasetImg(Dataset):
    ...
    def __init__(
        ...
        for idx in range(df.shape[0]):
            ...
            if mode.strip() in [
                'train_fullFT_noCard_gDescGeneration', 
                'train_partialFT_singleCard_gDescGeneration',
                'train_fullFT_categoryCard_gDescGeneration_v0',
                'train_partialFT_categoryCard_gDescGeneration_v0',
                ...
                inst = f'Instruction: Please generate a contextual description for the input image.'
                ip_stmt = f'{inst}\nCaption: {ocr}'                
                op_stmt = df.loc[idx, 'GENERAL_DESC'].strip()
            ...
            #
            if mode.strip() in [
                'testConf_fullFT_noCard_gDescGeneration',
                'testConf_partialFT_singleCard_gDescGeneration',

                'testConf_fullFT_categoryCard_gDescGeneration_v0',
                'testConf_partialFT_categoryCard_gDescGeneration_v0',
    ...
    def __getitem__(self, index):
        ...
        if self.execution_mode in [
            ...
            # logger.info(f'*** id_1: {id_}')
            # logger.info(f'*** img_id_1: {img_id}')
