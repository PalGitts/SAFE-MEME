...
class CustomDatasetImg(Dataset):
    ...
    def __init__(
        ...
        for idx in range(df.shape[0]):
            ...
            if mode.strip() in [
                'train_noGDescL1forImpOrExpDetectionTrain', 
                'train_withGDescL1forImpOrExpDetectionTrain'
            ]:
                ...
                if 'withGDesc' in mode: # For with GDesc
                    inst = f'Instruction: What kind of hate is present in the context w.r.t. the given caption and image description.'
                    ip_stmt = f'{inst}\nCaption: {ocr}\nImage description: {gdesc}'
                else:
                    inst = f'Instruction: What kind of hate is present in the context w.r.t. the given caption.'
                    ip_stmt = f'{inst}\nCaption: {ocr}'

                id_ = 'cnf_' +  df.loc[idx, 'IMG_ID'].strip().split('/')[-1].strip()
                
                op_stmt = 'Implicit [END]'
                if 'EXP' in label:
                    op_stmt = 'Explicit [END]'
            ...
            if mode.strip() in [
                ...
                # gdesc_json_path = f'./results/test_helperModuleGDesc.json'
                # saved_gdesc = json.load( open(gdesc_json_path) )
                gdesc = df.loc[idx, 'PRED_GENERAL_DESC'].strip()#saved_gdesc[id_]['pred_memeGDesc'].strip()
