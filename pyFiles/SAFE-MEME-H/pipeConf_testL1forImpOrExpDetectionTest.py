...
from utilConf_L1forImpOrExpDetectionTrain import img_shape, CustomDatasetImg
...
log_file = f"./logFiles/test_noGDescL1forImpOrExpDetectionInference.log"
...
def parse_args():
    ...
    mode_list = [
        ...
        'test_noGDescL1forImpOrExpDetectionInference', # ol 10
        'test_withGDescL1forImpOrExpDetectionInference', # ol-10
    ]
...
#         path = f'/home2/palash/M1/EMNLP_2024/usable_datasets/RMMHS_F/RMMHS_{idx_dataset}.xlsx' # Please path to DatasetA-Regular.xlsx, *** ATTENTION ***
...
def T5Trainer( args):
    ...
    if execution_mode in [
        'test_noGDescL1forImpOrExpDetectionInference',
        'test_withGDescL1forImpOrExpDetectionInference'
    ]:

        from model_singleCard import T5ForMultimodalGeneration
        logger.info(f'*** model_def: model_singleCard')  
            
        if 'withGDesc' in execution_mode:
            
            # json_L0 = f'./results/test_L0_usingfullFTCategoryCardbasedGDesc_v0.json'
            # json_name = f'test_L1_usingfullFTCategoryCardbasedGDesc_v0.json'

            # json_L0 = f'./results/test_L0_usingpartialFTCategoryCardbasedGDesc_v0.json'
            # json_name = f'test_L1_usingpartialFTCategoryCardbasedGDesc_v0.json'

            # json_L0 = f'./results/test_L0_usingfullFTCategoryCardbasedGDesc_v1.json'
            # json_name = f'test_L1_usingfullFTCategoryCardbasedGDesc_v1.json'
                  
            # json_L0 = f'./results/test_L0_usingpartialFTCategoryCardbasedGDesc_v1.json'
            # json_name = f'test_L1_usingpartialFTCategoryCardbasedGDesc_v1.json'
    
            #
            json_L0 = f'./resultsConf/testConf_L0_usingpartialFTCategoryCardbasedGDesc_v0.json'
            json_name = f'testConf_L1_usingfullFTCategoryCardbasedGDesc_v0.json'
        ...
        # else:
        #     json_name = f'test_noGDescL1forImpOrExpDetectionInference.json'
            
        #     chkp_id = 'checkpoint-903/'
        #     if args.img_type in ['vit', 'VIT']:
        #         model_dir = f'./unit_models/VIT_T5Base_fulllFT_train_noGDescL1forImpOrExpDetectionTrain_category_GENERAL/{chkp_id}'
        

        # gdesc_json_path = f'./results/test_fullFT_categoryCard_gDescGeneration_v0.json'
        # gdesc_json_path = f'./results/test_benignOrNOT_partialFT_singleCard_basedOn_fullFT_noCard_gDescGeneration.json'
        
        testCases_ExpOrImp = json.load( open(json_L0) )
        '''
            "rmmhs_image3533.jpg": {
            "seq_id": 1,
            "pred_label": "TRUE [END]",
            "pred_gdesc": "The first part of the image features a group of children and women walking down a dirt road, carrying water on their backs with the text 'these people have to walk 20 miles a day for fresh water'. The second part of the image depicts a white woman with her arms crossed, looking angry and crying. The associated text is 'then why don't they move closer to the fucking water?'. [END]",
            "OCR": "these people have to walk 20 miles a day for fresh water then why don't they move closer to the fucking water??",
            "LABEL": "EXPLICIT HATE",
            "PATH": "rmmhs_v2_images/image3533.jpg"
            },            from model_combinedCmodel_combinedCardQAGenerationTrain_v0ardQAGenerationTrain_v0 import T5ForMultimodalGeneration
            logger.info(f'*** model_def: model_combinedCardQAGenerationTrain_v0')  
            
            chkp_id = 'checkpoint-6450/'
            if args.img_type in ['vit', 'VIT']:
                model_dir = f'./unit_models/noGDesc/allQAGen/VIT_T5Base_fullFT_variationZERO_train_noGDescCombinedCardsAllQAGen_category_GENERAL/{chkp_id}'
        
            json_name = 'test_noGDescCombinedCardsAllQAGen_V0_output.json'
        ...
        for k, _dict in testCases_ExpOrImp.items():
            
            # print(_dict)
            if 'HATE' not in _dict['pred_label'].strip().upper():
                continue

            ocr_list.append(_dict['OCR'].strip())
            path_list.append(_dict['PATH'].strip())
            label_list.append(_dict['LABEL'].strip())
            pred_gdesc_list.append(_dict['pred_gdesc'].strip())

        data = { 'OCR': ocr_list, 'IMG_ID': path_list, 'LABEL': label_list, 'PRED_GENERAL_DESC': pred_gdesc_list}
        ...
        while case_processed < total_cases:
            ...
            if tester.is_world_process_zero():
                ...
                for idx, path_i in enumerate(temp_df['IMG_ID']):
                    ...
                    image_id = 'cnf_' + path_i.strip().split('/')[-1].strip()
