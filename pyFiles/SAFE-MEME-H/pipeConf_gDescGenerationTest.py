...
from utilConf_gDescGeneration import img_shape, CustomDatasetImg
...
log_file = f"./logFiles/test_partialFT_singleCard_gDescGeneration.log"
...
def parse_args():
    ...
    mode_list = [
        ...
        'testConf_fullFT_noCard_gDescGeneration', # ol - 125
        'testConf_partialFT_singleCard_gDescGeneration',

        'testConf_fullFT_categoryCard_gDescGeneration_v1',
        'testConf_partialFT_categoryCard_gDescGeneration_v1',

        'testConf_fullFT_categoryCard_gDescGeneration_v2', # Done M12
        'testConf_partialFT_categoryCard_gDescGeneration_v2', # M13
    ]
...
def get_df():
    ...
    path = f'/home2/palash/p0_ImplicitHateDetection/EMNLP_2024/usable_datasets/confounder_memes/TrickyConfounder.xlsx'
    ...
    logger.info(f'df is loaded: {df.shape}')
...
def T5Trainer( args):
    ...
    #
    if execution_mode in ['testConf_fullFT_noCard_gDescGeneration',]:

        from model_noCardTrain import T5ForMultimodalGeneration
        logger.info(f'*** model_def: model_singleCard')  
            
        json_name = f'testConf_gDescGeneration_fullFT_noCard.json'
        logger.info(f'**** json_name: {json_name}')
                        
        chkp_id = 'checkpoint-6450/'
        model_dir = f'./unit_models/VIT_T5Base_train_fullFT_noCard_gDescGeneration_category_GENERAL/{chkp_id}'
    ...
    if execution_mode in ['testConf_partialFT_singleCard_gDescGeneration',]:

        from model_singleCardTrain import T5ForMultimodalGeneration
        logger.info(f'*** model_def: model_singleCardTrain')  
            
        json_name = f'testConf_gDescGeneration_partialFT_singleCard.json'
        logger.info(f'**** json_name: {json_name}')
                        
        chkp_id = 'checkpoint-6450/'
        model_dir = f'./unit_models/VIT_T5Base_train_partialFT_singleCard_gDescGeneration_category_GENERAL/{chkp_id}'
        logger.info(f'***** model_dir: {model_dir}')
    ...
    if execution_mode in ['testConf_fullFT_categoryCard_gDescGeneration_v1']:
        ...
        json_name = f'testConf_gDescGeneration_fullFT_categoryCard_v0.json'
        logger.info(f'**** json_name: {json_name}')
                        
        chkp_id = 'checkpoint-3225/'
        model_dir = f'./unit_models/VIT_T5Base_train_fullFT_categoryCard_gDescGeneration_v0_category_GENERAL/{chkp_id}'
        logger.info(f'***** model_dir: {model_dir}')
    ...
    if execution_mode in ['testConf_partialFT_categoryCard_gDescGeneration_v1']:
        ...
        json_name = f'testConf_gDescGeneration_partialFT_categoryCard_v0.json'
        logger.info(f'**** json_name: {json_name}')
    ...
    if execution_mode in ['testConf_fullFT_categoryCard_gDescGeneration_v2']:
        ...
        json_name = f'testConf_gDescGeneration_fullFT_categoryCard_v1.json'
        logger.info(f'**** json_name: {json_name}')
    ...
    if execution_mode in ['testConf_partialFT_categoryCard_gDescGeneration_v2']: 
        ...
        json_name = f'testConf_gDescGeneration_partialFT_categoryCard_v1.json'
        logger.info(f'**** json_name: {json_name}')
    ...
    if execution_mode in [ 
        'testConf_fullFT_noCard_gDescGeneration', 'testConf_partialFT_singleCard_gDescGeneration',

        'testConf_fullFT_categoryCard_gDescGeneration_v1', 'testConf_partialFT_categoryCard_gDescGeneration_v1',

        'testConf_fullFT_categoryCard_gDescGeneration_v2', 'testConf_partialFT_categoryCard_gDescGeneration_v2',
    ]:
        test_df = get_df()
                ...
                for idx, path_i in enumerate(temp_df['IMG_ID']):
                    ...
                    if execution_mode in [
                        ...
                        id2info[image_id] = {
                            ...
                            'PATH': temp_df.loc[0,'IMG_ID'].strip(),
