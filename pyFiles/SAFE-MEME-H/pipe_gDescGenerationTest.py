...
def T5Trainer( args):
    ...
    #
    if execution_mode in ['test_fullFT_noCard_gDescGeneration',]:

        from model_noCardTrain import T5ForMultimodalGeneration
        logger.info(f'*** model_def: model_singleCard')  
            
        json_name = f'test_fullFT_noCard_gDescGeneration.json'
    ...
    if execution_mode in ['test_partialFT_singleCard_gDescGeneration',]:

        from model_singleCardTrain import T5ForMultimodalGeneration
        logger.info(f'*** model_def: model_singleCardTrain')  
            
        json_name = f'test_partialFT_singleCard_gDescGeneration.json'
        logger.info(f'**** json_name: {json_name}')
    ...
    if execution_mode in ['test_fullFT_categoryCard_gDescGeneration_v3']:
        ...
        model_dir = f'./unit_models/VIT_T5Base_train_fullFT_categoryCard_gDescGeneration_v2_category_GENERAL/{chkp_id}'
        logger.info(f'***** model_dir: {model_dir}')
