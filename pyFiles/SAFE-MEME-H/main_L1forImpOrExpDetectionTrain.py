...
log_file = f"./logFiles/train_withGDescL1forImpOrExpDetectionTrain.log"
...
def parse_args():
    ...
    # List of valid modes
    mode_list = [        
        'train_noGDescL1forImpOrExpDetectionTrain', # ol 10
        'train_withGDescL1forImpOrExpDetectionTrain', # ol-10
...
def T5Trainer(args):
    ...
    if execution_mode in [ 'train_noGDescL1forImpOrExpDetectionTrain', 'train_withGDescL1forImpOrExpDetectionTrain' ]:

        from model_singleCard import T5ForMultimodalGeneration
        
        save_dir = f'unit_models/{args.img_type.upper()}_T5Base_fulllFT_{execution_mode}_category_{args.category}'
        CARD_PATH = f'./trained_cards/card_train_ExpOrImp_partialFT_singleCard_{SAVE_EXT}'
        ...
        logger.info(f'***** CARD_PATH: {save_dir}')
    ...
    logger.info(f'eval_df: Unique target: { set(eval_df["TARGET"])}')
    ...
    logger.info(f'*** CARD is saved at CARD_PATH: {CARD_PATH}: {args.execution_mode}')
    logger.info(f'*** model is SAVED at: save_dir: {save_dir}: {args.execution_mode}')
