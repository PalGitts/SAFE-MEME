...
class CustomDatasetImg(Dataset):
    ...
    def __init__(
        ...
        for idx in range(df.shape[0]):
            ...
            if mode.strip() in ['testGDescContextGeneration']: # 256
                ...
                json_data = json.load( open(f'./results/test_memeGDescGeneration_output.json'))
            ...
            if mode.strip() in ['testGDescAllQAPairs']: # bs - 256
                ...
                print(f'CustomDatasetImg: {mode}: {dataset_type}')
                json_path = f'./results/test_memeGDescGeneration_output.json'
            ...
            if mode.strip() in ['testContextAllQAPairs']: # 256
                ...
                print(f'CustomDatasetImg: {mode}: {dataset_type}')
                json_path = f'./results/test_memeContextGeneration_output.json'
                json_data = json.load(open(json_path))
            ...
            if mode.strip() in ['test_QASummaryGeneration']: # bs- 350
            ...
            if mode.strip() in ['test_QASummaryToQApairsgeneration']: # bs-256
                
                print(f'CustomDatasetImg: {mode}: {dataset_type}')
                
                id_ = 'rmmhs_' +  df.loc[idx, 'PATH'].strip().split('/')[-1].strip()
                json_path = f'./results/test_QASummaryGeneration_output.json'
                
                json_data = json.load(open(json_path))
                qa_summary = json_data[id_]['QA_SUMMARY'].replace('[END]', '').strip()
            ...
            if mode.strip() in ['test_QASummaryToQuestionsGeneration']: # bs-256
            ...
            if mode.strip() in ['test_QASummaryQuestionsTo1Q1A']: # bs-85
    ...
    def __getitem__(self, index):
        ...
        if self.execution_mode in ['test_QASummaryQuestionsTo1Q1A']:
            id_ = 'rmmhs_' + self.df.loc[index, 'ID'].strip().split('/')[-1].strip()
            img_id = 'rmmhs_' + self.df.loc[index, 'PATH'].strip().split('/')[-1].strip()
        ...
        if self.execution_mode in ['test_QASummaryGeneration']: # bs - 350
            id_ = 'rmmhs_' + self.df.loc[index, 'PATH'].strip().split('/')[-1].strip()
            img_id = 'rmmhs_' + self.df.loc[index, 'PATH'].strip().split('/')[-1].strip()

        if self.execution_mode in ['test_QASummaryToQApairsgeneration']: # bs - 256
            id_ = 'rmmhs_' + self.df.loc[index, 'PATH'].strip().split('/')[-1].strip()
            img_id = 'rmmhs_' + self.df.loc[index, 'PATH'].strip().split('/')[-1].strip()

        
        if self.execution_mode in ['test_QASummaryToQuestionsGeneration']: # bs - 128
            id_ = 'rmmhs_' + self.df.loc[index, 'PATH'].strip().split('/')[-1].strip()
            img_id = 'rmmhs_' + self.df.loc[index, 'PATH'].strip().split('/')[-1].strip()
